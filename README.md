...
rtt min/avg/max/mdev = 58.625/58.625/58.625/0.000 ms
...
1. Honeypot Deployed: Dionaea
  - [ ] Summary:
    ...
    - Number of Unique IPs: cat session.json | awk -F "," '{print $5}' | sort | uniq -c |sort -r |wc -l == ```64```
    ...
    - Protocols: cat session.json | awk -F "," '{print $2}' | sort | uniq -c | wc == ```7```
        1055  "protocol" : "pcap"
        25  "protocol" : "httpd"
